...
function notEmptyNotStarNotComposite(item) {
  return notEmpty(item.expression) &&
    item.expression !== "*" &&
    !item.isComposite;
}
...
function notEmpty(item) {
  switch (typeof item) {
    ...
    case "object":
      return item !== null &&
        (typeof item.getDay === "function" || Object.keys(item).length > 0); // jscs:ignore maximumLineLength
  }
}
...
function _findIndexOfColumn(columns, targetColumn) {
  return columns.reduce(
    function(colIndex, col, i) {
      var containsField = col.rawColumn === targetColumn ||
        col.column === targetColumn;
      ...
      if (isNaN(number)) {
        ...
        if (isNaN(num)) {
          return formatFilterValue(
            moment().utc().startOf(datepart).toDate(),
            true
          );
        } else {
          return formatFilterValue(
            moment().add(num, datepart).utc().startOf(datepart).toDate(),
            true
          );
        }
      } else {
  ...
  function crossfilter() {
        ...
        _dataConnector.getFields(table, (error, columnsArray) => {
          if (error) {
            ...
            columnsArray.forEach(function(element) {
              ...
              columnNameCountMap[element.name] = columnNameCountMap[
                element.name
              ] === undefined
                ? 1
    ...
    function dimension(expression, isGlobal) {
      ...
      function filterNotLike(value, append) {
        if (append) {
          scopedFilters[dimensionIndex] += "NOT( " +
      ...
      function formatRelativeValue(val) {
        if (val.now) {
          ...
          const operator = typeof val.operator !== "undefined"
            ? val.operator
            : "DATE_ADD";
      ...
      function filterMulti(
        ...
        var filterWasNull = filters[dimensionIndex] == null ||
        ...
        inverseFilters = typeof inverseFilters === "undefined"
          ? false
          : inverseFilters;
      ...
      function writeQuery(hasRenderSpec) {
        ...
        if (hasRenderSpec) {
          ...
          if (
            projList.indexOf("rowid") < 0 && projList.indexOf(rowIdAttr) < 0
          ) {
        ...
        function getBinnedDimExpression(
          ...
          if (isDate) {
            if (timeBin) {
              ...
              var filterRange = (binBounds[1].getTime() -
                binBounds[0].getTime()) *
                0.001;
        ...
        function reduce(expressions) {
          ...
          for (var e = 0; e < numExpressions; e++) {
            ...
            if (
              ...
              reduceExpression += " AVG(CASE WHEN " +
                filters[targetFilter] +
                " THEN 1 ELSE 0 END)";
            } else {
              /*
              ...
              if (agg_mode === "CUSTOM") {
                ...
                if (expressions[e].filter) {
                  reduceExpression += "COUNT(CASE WHEN " +
      ...
      for (var d = 0; d < dimArray.length; d++) {
        if (dimArray[d] in columnTypeMap) {
          ...
          dimContainsArray[d] = columnTypeMap[
            compoundColumnMap[dimArray[d]]
          ].is_array;
        } else {
