  ...
  function crossfilter() {
        ...
        function writeQuery(
          ...
          for (var i = 0; i < tempArray.length; i++) {
            ...
            if (!!hasRenderSpec && tempArray[i].match(/rowid\s*$/)) {
              ...
              if (
                typeof columnTypeMap[
                  _tablesStmt + "." + tempArray[i].toUpperCase()
                ] !== "undefined"
              ) {
                if (
                  columnTypeMap[_tablesStmt + "." + tempArray[i].toUpperCase()]
                    .is_array &&
                  !tempArray[i].match(/unnest\s*$/i)
                ) {
          ...
          if (
            ...
            for (var d = 0; d < arrayDataQueryParams.length; d++) {
              if (arrayDataQueryParams[d] !== null) {
                ...
                hasBinParams = true
                havingSubClause += `${arrayDataQueryParams[d].column} ${
                  arrayDataQueryParams[d].operator
                } ${arrayDataQueryParams[d].value}`
    ...
    function groupAll() {
      ...
      function writeQuery(ignoreFilters, ignoreChartFilters) {
        ...
        /* Code added by Sachin to support group by - having clause */
        if (
          typeof arrayDataQueryParams !== "undefined" &&
          arrayDataQueryParams !== null &&
          arrayDataQueryParams.length > 0
        ) {
          query += " GROUP BY "
          ...
          dimColsArray = dimColsArray.filter((v, i, a) => a.indexOf(v) === i)
        }
        ...
        for (var i = 0; i < dimColsArray.length; i++) {
          ...
          if (
            typeof columnTypeMap[
              _tablesStmt + "." + dimColsArray[i].toUpperCase()
            ] !== "undefined"
          ) {
            if (
              columnTypeMap[_tablesStmt + "." + dimColsArray[i].toUpperCase()]
                .is_array &&
              !dimColsArray[i].match(/unnest\s*$/i)
            ) {
