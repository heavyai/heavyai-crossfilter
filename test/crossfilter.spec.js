...
describe("crossfilter", () => {
  ...
  describe(".setDataAsync", function() {
    ...
    it("identifies ambiguous table columns", () => {
      ...
      getFieldsReturnValue = { columns: columnsArray }
      const dataConnector = { getFieldsAsync }
    ...
    describe(".writeTopQuery", () => {
      ...
      it("can use a samplingRatio < 1 with filterQuery", () => {
        ...
        expect(dimension.writeTopQuery(Infinity)).to.eq(
          "SELECT users.id FROM users WHERE  MOD(MOD(users.rowid, 2147483648) * 2654435761, 4294967296) < 2147483648 ORDER BY users.id DESC"
      ...
      it("can use a samplingRatio < 1 without filterQuery", () => {
        ...
        dimension.samplingRatio(0.5)
        expect(dimension.writeTopQuery(Infinity)).to.eq(
          "SELECT users.id FROM users WHERE users.id = 1 AND  MOD(MOD(users.rowid, 2147483648) * 2654435761, 4294967296) < 2147483648 ORDER BY users.id DESC"
      ...
      it("can use a join statement with no filterQuery or samplingRatio", () => {
        ...
        dimension.samplingRatio(0.5)
        expect(dimension.writeTopQuery(Infinity)).to.eq(
          "SELECT users.id FROM tableA WHERE  MOD(MOD(tableA.rowid, 2147483648) * 2654435761, 4294967296) < 2147483648 AND table1.id = table2.x_id ORDER BY users.id DESC"
        ) // TODO extra space between WHERE & MOD
      })
      ...
      describe(".size", () => {
        ...
        it("returns number of records", () => {
          crossfilter.setDataAsync(
            {
              getFieldsAsync: () => Promise.resolve([]),
              query: () => [{ n: 123 }]
            },
            null,
            []
